...
class EncoderDecoder1DCNN(nn.Module):
    ...
    def conv_layer(self, in_channels : int,
                   ...
                   dilation : int,
                   l_in : int,
                   transposed : bool = False) -> Tuple[nn.Module, int]:

        if transposed:
            conv_ = nn.ConvTranspose1d(in_channels, out_channels, kernel_size, stride, padding)
            l_out = (l_in-1)*stride - 2*padding + dilation*(kernel_size-1) + 1

            return conv_, int(l_out)

        conv_ = nn.Conv1d(in_channels, out_channels, kernel_size, stride, padding, dilation)
    ...
    def get_encoder(self) -> nn.Sequential:
        ...
        conv2, l_out = self.conv_layer(5, 10, 10, 1, 9, 1, l_out)
        conv3, l_out = self.conv_layer(10, 10, 10, 1, 9, 1, l_out)
        conv4, l_out = self.conv_layer(10, 10, 10, 1, 9, 1, l_out)
        conv5, l_out = self.conv_layer(10, 1, 10, 1, 9, 1, l_out)

        linear = nn.Linear(l_out, self.encoding_len)

        encoder = nn.Sequential(
            conv1, nn.SELU(), conv2, nn.SELU(),
            conv3, nn.SELU(), conv4, nn.SELU(),
            conv5, nn.SELU(), linear)
    ...
    def get_decoder(self) -> Tuple[nn.Sequential, int]:
        ...
        conv2, l_out = self.conv_layer(5, 10, 10, 1, 0, 1, l_out, True)
        conv3, l_out = self.conv_layer(10, 10, 10, 1, 0, 1, l_out, True)
        conv4, l_out = self.conv_layer(10, 5, 10, 1, 0, 1, l_out, True)
